 ...
 * @returns {RegExp[]}
 */
const getRouteRexps = (route, params) => {
 ...
 * @returns {RegExp[]}
 */
const getCacheConfRexps = (cacheConf, params) => {
  const rexps = [];
  ...
  for (const route of routes) {
    rexps.push(...getRouteRexps(route, params));
  }
  ...
  return {
    ...
    initialize() {
      ...
      const clearCache = async (cacheConf, params = {}) => {
        ...
        const rexps = getCacheConfRexps(cacheConf, params);
        ...
        if (options.clearRelatedCache) {
          ...
          for (const uid of relatedModelsUid) {
            ...
            if (relatedCacheConf) {
              rexps.push(...getCacheConfRexps(relatedCacheConf))
            }
          }
        }
