 ...
 * @property {boolean=} disableHitpass
 ...
 * @returns {RegExp[]}
 */
const getRouteRegExp = (route, params) => {
 ...
 * @returns {RegExp[]}
 */
const getCacheConfRegExp = (cacheConf, params) => {
  const regExps = [];
  ...
  for (const route of routes) {
    regExps.push(...getRouteRegExp(route, params));
  }
...
const Cache = (strapi) => {
  ...
  /**
   * Get related ModelCacheConfig to a model uid
   *
   * @param {string} uid
   * @returns {ModelCacheConfig=}
   */
   ...
   * @return {string[]} Array of related models
   */
  const getRelatedModelsUid = (cacheConf) => {
    ...
    const modelKey =
      _.findKey(models, { collectionName: cacheConf.model }) || cacheConf.model;
  ...
  return {
    ...
    initialize() {
      ...
      const clearCache = async (cacheConf, params = {}) => {
        ...
        if (options.clearRelatedCache) {
          ...
          for (const uid of relatedModelsUid) {
            ...
            if (relatedCacheConf) {
              regExps.push(...getCacheConfRegExp(relatedCacheConf));
            }
          }
        }
      ...
      const recv = (cacheConf) => async (ctx, next) => {
        ...
        // hitpass if Authorization or Cookie header are present
        const lookup = (cacheConf.disableHitpass && true) || !(ctx.request.headers.authorization || ctx.request.headers.cookie)
        ...
        // fetch done
        if (!lookup) {
          logger.debug(`GET ${ctx.path} ${chalk.yellow("HITPASS")}`);
